...
| CI/CD | GitHub Actions workflow `CI` (`.github/workflows/ci.yml`) on pushes to `main` and pull requests targeting it: `npm ci`, `npm run check`, `npm test`, `npm pack`, and `node scripts/verify-package.mjs` on the packed tarball, across Ubuntu with Node.js 22 and 24 and macOS with Node.js 22. Because `@wonder-wagon/tokens` and `@wonder-wagon/themes` are `file:` devDependencies on a sibling checkout until they publish, the workflow first clones `rikilamadrid/wonder-wagon-ui` beside the repository at the commit pinned as `WONDER_WAGON_UI_REF` and builds those two packages with Bun, so `npm run wonder-wagon:check` runs for real in CI; bump the ref whenever `npm run wonder-wagon:sync` is re-run. `main` requires the `CI` check with strict up-to-date branches. No publishing from CI. |
