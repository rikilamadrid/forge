...
| CI/CD | GitHub Actions workflow `CI` (`.github/workflows/ci.yml`) on pushes to `main` and pull requests targeting it: `npm ci`, `npm run check`, `npm test`, `npm pack`, and `node scripts/verify-package.mjs` on the packed tarball, across Ubuntu with Node.js 22 and 24 and macOS with Node.js 22. Because the React-free `wonder-wagon-ui/cli` generator is a `file:` devDependency on a sibling checkout until it publishes, the workflow first clones `rikilamadrid/wonder-wagon-ui` beside the repository at the commit pinned as `WONDER_WAGON_UI_REF` and builds `packages/foundation` with Bun, so `npm run wonder-wagon:check` runs against the exact source which generated the committed runtime; bump the ref whenever `npm run wonder-wagon:sync` is re-run. `main` requires the `CI` check with strict up-to-date branches. No publishing from CI. |
...
| 2026-09-22 | The Forge website's canonical production URL is `https://forge-kit-nu.vercel.app`, observed from the Vercel project `forge-kit` (`prj_NBg9Fpt9yvYBaFbHXGLrc5FLvnM9`, team `rikilamadrids-projects`) after its first successful production deployment, built from `357331a`. It is the project's only configured domain, verified, with no custom domain and no redirect. The Feature 05 spec's *expected* `https://forge-kit.vercel.app` is **not** this project: that subdomain is held by an unrelated Vercel account and serves a placeholder page, which is why Vercel assigned the `-nu` suffix. Two assumptions were tested against the Vercel API while observing this and both are false. There is no duplicate Vercel project: `forge-kit` is the only project linked to `rikilamadrid/forge`, and the `forge-<hash>-<scope>.vercel.app` hosts are its own deployment URLs, not a second project's. Production is not blocked by Deployment Protection: `ssoProtection` is `all_except_custom_domains`, which exempts the canonical production domain, so `forge-kit-nu.vercel.app` returns `200` unauthenticated while the auto-generated aliases `forge-kit-rikilamadrids-projects.vercel.app` and `forge-kit-git-main-rikilamadrids-projects.vercel.app` return `302` to Vercel SSO. | This URL is the one every public surface must carry, and ticket `05.6` propagates it to `package.json` `homepage`, the README, and the repository About link — so the value it reconciles against has to be the observed one, not the expected one. Recording the two disproven assumptions beside it stops a later ticket re-diagnosing a duplicate project that does not exist, or changing a Vercel setting that needs no change. |
| 2026-09-22 | The public website's Open Graph metadata is the automated, version-controlled source of truth for Forge's social presentation. A custom GitHub repository social-preview image is an optional human-only enhancement and blocks nothing; GitHub's generated repository card is acceptable. Verified the same day that GitHub exposes no supported mechanism to set that image — none of its 259 GraphQL mutations matches `opengraph`/`social`/`preview`/`image`, `UpdateRepositoryInput` has no image field, the plausible REST paths all return `404`, a `PATCH /repos/{owner}/{repo}` carrying `social_preview` or `open_graph_image` returns `200` and is silently ignored, and `gh repo edit` has no such flag — with a token holding `repo`, so this is a missing capability, not a permissions gap. The only working path is a session-cookie-authenticated upload through GitHub's settings form, which is deliberately outside this project's automated workflow. | Social presentation has to be reviewable, replayable, and CI-verifiable, and the website's `og:image` and `twitter:card` are all three because they live in `site/src/pages/index.astro` and build inside the required `CI` check. Hanging a Feature on a browser-only upload no API can perform would stall delivery on a step the project cannot automate or verify, so the dependency is removed rather than faked. Recording the investigation stops a later ticket re-testing an API that does not exist. |
