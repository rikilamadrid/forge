...
## Forge TypeScript Conventions

- Use strict TypeScript and ESM. Do not weaken compiler checks to make an
  integration compile.
- Prefer Node.js platform APIs over production dependencies. A dependency must
  solve a demonstrated gap and requires human approval.
- Keep provider-neutral request, result, metric, and error types independent of
  Ollama wire types.
- Validate environment configuration and untrusted HTTP payloads at their
  boundaries. Do not use unchecked type assertions for provider responses.
- Inject the HTTP operation and time source where tests need control; do not add
  a dependency-injection framework.
- Keep units explicit in names. Ollama duration values are nanoseconds at the
  wire boundary; normalize public duration values to milliseconds.
- Model expected failures with stable error categories. Do not inspect error
  message text to drive program behavior.
- Keep CLI parsing and presentation separate from the ask use case.
- Use `node:test` and behavior-focused tests. Live Ollama verification is
  opt-in and must never be part of the deterministic default test command.
- Do not log or persist prompt or response content except for the response the
  invoking user explicitly requested.
