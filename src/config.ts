...
export function loadConfig(environment: Environment): ForgeConfig {
  ...
  if (!hostValue) {
    throw new Error("OLLAMA_HOST is required. Set it to the Ollama API base URL.");
  }
  ...
  if (!model) {
    throw new Error("FORGE_MODEL is required. Set it to an installed Ollama model name.");
  }
